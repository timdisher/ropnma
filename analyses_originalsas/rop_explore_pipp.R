...
pa_reac_netgraph = pub_netgraph(chars, nodecolour = c("#9A32CD"))
...
pa_reac_graph =  pairwise(data = pa_reac,treat = trt_group, n= sample_size, mean = mean,sd = std_dev,studlab = studlab) %>%
  mutate(TE = TE*-1) %>% left_join(pa_reac[c("studlab","imputed_mean","scaled_score")] %>% distinct(), by = "studlab") %>%left_join(rop_data_study[c("studlab","design","avg_pma","oa_rob_sub","avg_bw")], by = "studlab") %>% 
...
reg_bars = function(data = pa_reac_graph, labeller = treatment_names){

plots = list(ctrl = nma_bargraphs(data = data,outcome = "mean1", labeller = labeller) + labs(y = "Control group PIPP score")+ ggtitle(""),
             
             pma = nma_bargraphs(data = data,outcome = "avg_pma", labeller = labeller) + labs(y = "Average PMA (weeks)")+ ggtitle(""),
             
             bw = nma_bargraphs(data = data,outcome = "avg_bw", labeller = labeller) + labs(y = "Average birthweight (grams)")+ ggtitle(""),
             
             im = nma_bargraphs(data = data,outcome = "mean2", labeller = labeller)+ geom_col(aes(fill = imputed_mean)) + 
               scale_fill_manual(name = "Imputed mean", values = c("darkorchid","lightblue"),labels = c("No","Yes")) + labs(y = "Treatment mean PIPP") + ggtitle(""),
             
             xover = nma_bargraphs(data = data,outcome = "mean2", labeller = labeller)+ geom_col(aes(fill = design)) + ggtitle("") +
               scale_fill_manual(name = "Design", values = rev(c("darkorchid","lightblue")),labels = c("Crossover","Parallel")) + labs(y = "Treatment mean PIPP"),
             
             scaled = nma_bargraphs(data = data,outcome = "mean2", labeller = labeller)+ geom_col(aes(fill = scaled_score)) +
               scale_fill_manual(name = "Scaled score", values = c("darkorchid","lightblue"),labels = c("No","Yes")) + labs(y = "Treatment mean PIPP") + ggtitle(""),
             
             rob = nma_bargraphs(data = data,outcome = "mean2", labeller = labeller)+ geom_col(aes(fill = oa_rob_sub)) +
               scale_fill_manual(name = "Risk of Bias", values = rev(c("darkorchid","lightblue")),labels = c("High","Low")) + labs(y = "Treatment mean PIPP") + ggtitle("")
...
pa_reac_bars = reg_bars()

pdf("./figs/pain scales reactivity/pa_reac_bars.pdf", height = 8.5, width = 11)
invisible(lapply(pa_reac_plots,print))
dev.off()
...
### Generate network graph
pa_recov_netgraph = pub_netgraph(recov_chars, nodecolour = c("#9A32CD"))
...
pa_recov_graph =  pairwise(data = pa_recov,treat = trt_group, n= sample_size, mean = mean,sd = std_dev,studlab = studlab) %>%
  mutate(TE = TE*-1) %>% left_join(pa_recov[c("studlab","imputed_mean","scaled_score")] %>% distinct(), by = "studlab") %>%left_join(rop_data_study[c("studlab","design","avg_pma","oa_rob_sub","avg_bw")], by = "studlab") %>% 
  replace_na(replace = list(avg_bw = 0, imputed_mean = "no",scaled_score = "no",avg_bw = 0)) %>% mutate(oa_rob_sub = ifelse(oa_rob_sub == "low","low","high"))
...
treatment_names_recov = c("label_value")
pa_recov_bars = reg_bars(pa_recov_graph, labeller = treatment_names_recov)

pdf("./figs/pain scales recovery/pa_recov_bars.pdf", height = 8.5, width = 11)
invisible(lapply(pa_recov_bars,print))
dev.off()
### meta-regression plots
